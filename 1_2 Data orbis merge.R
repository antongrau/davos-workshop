...
set.orbis <- orbis %>% select(Organization, `Nom de l'entreprise`, `Number of employees (last value) - Date`, `NACE Rev.2 Code principal`, Ville, `Code ISO Pays`, `Date de création`, `Total de l'Actif (Dernière année) mUSD (Taux de change à la dernière date de clôture)` )
write_csv(set.orbis, path = "output/orbis_batch_upload.csv")
